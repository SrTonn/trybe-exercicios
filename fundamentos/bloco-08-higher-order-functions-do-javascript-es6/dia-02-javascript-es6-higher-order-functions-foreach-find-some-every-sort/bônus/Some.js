...
const qualComercial = (arr) => {
  const isChild = arr.some(age => age < 18)
  return isChild ? "ChocoTrybe, melhor que console.log" : "Beba Trybeer, suave como um log de erro vazio"
};

console.log(qualComercial(arrayIdades)); // ChocoTrybe, melhor que console.log
console.log(qualComercial(arrayIdades2)); // Beba Trybeer, suave como um log de erro vazio
...
const vaiTerMentoria = (array) => {
  const found = array.some(({status}) => status < 100)
  return found ? 'Bora pra mentoria de projetos!' : 'Pode abrir o suco de laranja que temos 100% de aprovação!'
};

console.log(vaiTerMentoria(estudantes)); // Bora pra mentoria de projetos!
