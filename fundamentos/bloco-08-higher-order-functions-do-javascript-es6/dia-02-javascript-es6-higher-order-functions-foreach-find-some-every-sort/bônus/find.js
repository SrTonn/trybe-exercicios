...
const msgbotProjeto = (array) => {
  const objFound = array.find(({status}) => status < 100)
  return objFound ? `Olá ${objFound.nome}, o prazo para entrega do projeto é daqui a 3 dias, vc precisa de alguma ajuda?` : ''
};

const res = msgbotProjeto(estudantes);
res
...
const msgbotPedidoAjuda = (array) => {
  const objFound = array.find(({status}) => status === 100)
  const message = `Olá ${objFound.nome}, \
parabéns por ter finalizado 100% do projeto, \
o que vc acha de ajudar outras pessoas da turma a tbm entregarem o projeto?`
  if (objFound) return message
};

console.log(msgbotPedidoAjuda(estudantes));
