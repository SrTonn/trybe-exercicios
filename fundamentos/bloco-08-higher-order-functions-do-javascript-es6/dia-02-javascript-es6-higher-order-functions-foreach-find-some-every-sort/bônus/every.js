...
const qualComercial = (arr) => {
  ...
  return isAllAdults ? 'Beba Trybeer, suave como um log de erro vazio' : 'ChocoTrybe, melhor que console.log';
};

console.log(qualComercial(arrayIdades));
console.log(qualComercial(arrayIdades2));
...
const vaiTerMentoria = (array) => {
  const isAllApproved = array.every(({status}) => status === 100)
  return isAllApproved ? "Pode abrir o suco de laranja que temos 100% de aprovação!" : "Bora pra mentoria de projetos!"
};

console.log(vaiTerMentoria(estudantes));
